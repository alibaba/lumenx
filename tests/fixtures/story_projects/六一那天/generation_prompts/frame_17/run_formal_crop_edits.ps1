...
$CropEdits = @(
  @{
    Label = "adult_xiaoqi"
    PromptFile = "02a_adult_xiaoqi_crop_edit_prompt.txt"
    InputCrop = "liuyi_frame_17_stage1_base_crop_adult_xiaoqi.png"
    ReferenceImage = "liuyi_char_xiaoqi_adult_full_body.png"
    OutputPath = "liuyi_frame_17_stage2_adult_xiaoqi_formal_v1.png"
    Size = "768x896"
  }
)
...
foreach ($edit in $CropEdits) {
  Invoke-FormalCropEdit `
    -Label $edit["Label"] `
    -PromptFile (Join-Path $FrameDir $edit["PromptFile"]) `
    -InputCrop (Join-Path $GeneratedDir $edit["InputCrop"]) `
    -ReferenceImage (Join-Path $FixtureDir $edit["ReferenceImage"]) `
    -OutputPath (Join-Path $GeneratedDir $edit["OutputPath"]) `
    -Size $edit["Size"]
}
