...
export default function VideoAssembly() {
    ...
    const handleSelectVideo = async (frameId: string, videoId: string) => {
        ...
        try {
            ...
            alert(copy.failedToSelectVideo);
        }
    };
    ...
    const handleMerge = async () => {
        ...
        try {
            ...
            const errorDetail = extractErrorDetail(error, copy.unknownMergeError);

            setMergeError(errorDetail);

            // Also show alert for immediate feedback
            alert(`${copy.failedToMergeVideos}:\n\n${errorDetail}`);
        } finally {
            setIsMerging(false);
        }
    };
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            const url = getAssetFetchUrl(rawPath);

            if (isDirectAssetPath(rawPath)) {
                                    ...
                                    <div className="flex-1 p-4 flex flex-col justify-between min-w-0">
                                        <div className="space-y-2">
                                            <div className="flex items-start gap-2">
                                                ...
                                                <p className="text-sm text-gray-300 line-clamp-2 leading-relaxed">
                                                    {frame.image_prompt || frame.action_description || copy.noPrompt}
                                                </p>
                                            </div>
                                            {frame.dialogue && (
                                        ...
                                        <div className="flex items-center justify-between mt-2 pt-2 border-t border-white/5">
                                            <div className="flex items-center gap-4 text-xs text-gray-500">
                                                ...
                                                <span className="flex items-center gap-1">
                                                    <Film size={12} /> {videosByFrame[frame.id]?.length || 0} {copy.variants}
                                                </span>
                                            </div>

                                            {selectedVideoId && (
                                                <div className="flex items-center gap-1 text-green-500 text-xs font-bold">
                                                    <Check size={12} /> {copy.ready}
                                                </div>
                                            )}
                                        </div>
                                    </div>
                            ...
                            <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4">
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1">
                                        <h4 className="text-sm font-bold text-red-400 mb-1">{copy.mergeFailed}</h4>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-4">
                        {selectedFrameId ? (
                            <div className="space-y-4">
                                {variants.length > 0 ? (
                                    ...
                                    <div className="text-center py-12 text-gray-500 flex flex-col items-center">
                                        ...
                                        <p className="text-sm font-medium">{copy.noVideosGenerated}</p>
                                        <p className="text-xs mt-1 max-w-[200px]">{copy.goBackToMotion}</p>
                                    </div>
                                )}
                            </div>
                        ) : (
                            <div className="h-full flex flex-col items-center justify-center text-gray-500 gap-3">
                                ...
                                <p className="text-sm">{copy.selectFrameHint}</p>
                            </div>
                        )}
                    </div>
                </div>
                            ...
                            <div className="flex-1 flex flex-col justify-center space-y-4">
                                <div>
                                    <h3 className="text-xl font-bold text-white flex items-center gap-2">
                                        <Check className="text-green-500" /> {copy.mergedVideoReady}
                                    </h3>
                                ...
                                <div className="flex gap-4">
                                    ...
                                    >
                                        <Download size={18} />
                                        {isDownloading ? copy.downloading : copy.downloadMp4}
                                    </button>
